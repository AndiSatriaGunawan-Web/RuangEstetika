...
function renderTestimonials() {
    const testimonials = [
        ...
        { name: "Rina Kartika", location: "Semarang", rating: 5, text: "Kasur spring bed nya enak banget, tidur jadi nyenyak. Recommended untuk yang punya masalah punggung." }
    ];
...
function startCountdown() {
    ...
    function update() {
        ...
        if (countdownEl) {
            ...
            gsap.utils.toArray(".countdown span").forEach(span => {
                gsap.fromTo(span, { scale: 0.8, opacity: 0.5 }, { scale: 1, opacity: 1, duration: 0.3, ease: "back.out(0.5)" });
            });
        }
    }
    update();
...
function setupEventListeners() {
    ...
    function showWishlistModal() {
        ...
        if (state.wishlist.length === 0) {
            wishlistBody.innerHTML = '<div style="text-align: center; padding: 40px;"><i class="far fa-heart" style="font-size: 3rem; color: #ccc;"></i><p style="margin-top: 16px;">Belum ada produk di wishlist</p><button class="btn-outline" id="goToCatalogWishlist" style="margin-top: 16px;">Jelajahi Produk</button></div>';
            const goToCatalog = document.getElementById('goToCatalogWishlist');
            if (goToCatalog) goToCatalog.addEventListener('click', () => { wishlistModal.classList.remove('open'); navigateTo('catalog'); });
        } else {
            ...
            wishlistBody.innerHTML = wishlistProducts.map(p => `
                <div style="display: flex; gap: 12px; margin-bottom: 15px; padding-bottom: 15px; border-bottom: 1px solid #eee;">
                    <img src="${p.image}" style="width: 60px; height: 60px; object-fit: cover; border-radius: 8px; cursor: pointer;" class="wishlist-product-click" data-id="${p.id}">
                    <div style="flex: 1;">
                        <div style="font-weight: 600; cursor: pointer;" class="wishlist-product-click" data-id="${p.id}">${p.name}</div>
            ...
            document.querySelectorAll('.wishlist-product-click').forEach(el => el.addEventListener('click', () => { const id = parseInt(el.dataset.id); showProductModal(id); wishlistModal.classList.remove('open'); }));
        }
