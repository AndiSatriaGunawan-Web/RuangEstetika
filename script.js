/* ============================================
   ...
   Version: 4.0.0 - FULLY FIXED WITH GSAP
   ============================================ */
...
function isInWishlist(productId) { return state.wishlist.includes(productId); }

function updateWishlistUI() {
    const total = state.wishlist.length;
    document.querySelectorAll('#wishlistCount, #bottomWishCount, #mobileWishCount').forEach(el => { if (el) el.textContent = total; });
}
...
function renderCategories() {
    ...
    const icons = { 'Meja': 'fa-chair', 'Kursi': 'fa-couch', 'Sofa': 'fa-loveseat', 'Kasur': 'fa-bed', 'Lampu': 'fa-lightbulb', 'Dekorasi': 'fa-palette' };
    const container = document.getElementById('categoriesGrid');
    if (!container) return;
    
    container.innerHTML = categories.map(cat => `<div class="category-card" data-category="${cat}"><i class="fas ${icons[cat]}"></i><h3>${cat}</h3><p>${state.products.filter(p => p.category === cat).length} produk</p></div>`).join('');
    ...
    return `
        <div class="product-card" data-id="${product.id}">
            ...
            <div class="product-info">
                ...
                <div class="product-rating">${'★'.repeat(Math.floor(product.rating))}${product.rating % 1 >= 0.5 ? '½' : ''}${'☆'.repeat(5 - Math.ceil(product.rating))}<span>(${product.reviews})</span></div>
                <div class="product-price">${formatRupiah(product.price)}${product.oldPrice ? `<span style="text-decoration: line-through; color: #999; font-size: 0.8rem; margin-left: 8px;">${formatRupiah(product.oldPrice)}</span>` : ''}</div>
                <button class="add-to-cart" data-id="${product.id}"><i class="fas fa-shopping-cart"></i> Tambah ke Keranjang</button>
            </div>
        </div>
    `;
}
...
function renderInspirationGrid() {
    ...
    if (container) {
        container.innerHTML = inspirations.map(insp => `<div class="inspiration-card"><img src="${insp.image}" alt="${insp.title}" loading="lazy"><div class="inspiration-overlay"><h3>${insp.title}</h3><p>${insp.style}</p></div></div>`).join('');
    }
}

function renderTestimonials() {
    const testimonials = [
        { name: "Siti Rahma", location: "Jakarta", rating: 5, text: "Pelayanan sangat baik, produk berkualitas. Meja yang saya beli sangat kokoh!" },
        { name: "Budi Santoso", location: "Surabaya", rating: 5, text: "Sofa nya sangat nyaman, cocok untuk keluarga. Recommended banget!" },
    ...
    if (container) {
        container.innerHTML = testimonials.map(t => `<div class="testimonial-card"><i class="fas fa-quote-left"></i><p>"${t.text}"</p><div style="margin-top: 15px;"><strong>${t.name}</strong><p style="font-size: 0.75rem; color: #999;">${t.location}</p><div style="color: #FFB800;">${'★'.repeat(t.rating)}${'☆'.repeat(5 - t.rating)}</div></div></div>`).join('');
    }
}
...
function attachProductEvents() {
    ...
    document.querySelectorAll('.product-card').forEach(card => { card.removeEventListener('click', handleProductClick); card.addEventListener('click', handleProductClick); });
}

function handleAddToCart(e) { e.stopPropagation(); const id = parseInt(e.currentTarget.dataset.id); addToCart(id); }
function handleProductClick(e) { if (e.target.classList.contains('add-to-cart')) return; const id = parseInt(e.currentTarget.dataset.id); showProductModal(id); }
...
function showProductModal(productId) {
    ...
    modalBody.innerHTML = `<div style="text-align: center;"><img src="${product.image}" alt="${product.name}" style="width: 100%; border-radius: 16px; margin-bottom: 16px;"><h3>${product.name}</h3><div class="product-category">${product.category}</div><div class="product-rating" style="justify-content: center; margin: 10px 0;">${'★'.repeat(Math.floor(product.rating))}${product.rating % 1 >= 0.5 ? '½' : ''} (${product.reviews} ulasan)</div><div class="product-price" style="font-size: 1.5rem; margin: 16px 0;">${formatRupiah(product.price)}${product.oldPrice ? `<span style="text-decoration: line-through; font-size: 1rem; margin-left: 8px;">${formatRupiah(product.oldPrice)}</span>` : ''}</div><p style="margin-bottom: 20px; color: #666;">${product.description}</p><div style="margin-bottom: 16px; color: ${product.stock > 0 ? 'green' : 'red'};"><i class="fas ${product.stock > 0 ? 'fa-check-circle' : 'fa-times-circle'}"></i>${product.stock > 0 ? `Stok tersedia (${product.stock} unit)` : 'Stok habis'}</div><button class="btn-primary" id="modalAddToCart" style="width: 100%;">Tambah ke Keranjang</button></div>`;
...
function navigateTo(page) {
    ...
    if (breadcrumbCurrent) {
        const pageNames = { home: 'Beranda', catalog: 'Katalog', promo: 'Promo', inspiration: 'Inspirasi', testimonial: 'Testimoni', contact: 'Kontak' };
...
function startCountdown() {
    ...
    function update() {
        ...
        if (countdownEl) countdownEl.innerHTML = `<div><span>${String(days).padStart(2, '0')}</span><small>Hari</small></div><div><span>${String(hours).padStart(2, '0')}</span><small>Jam</small></div><div><span>${String(minutes).padStart(2, '0')}</span><small>Menit</small></div><div><span>${String(seconds).padStart(2, '0')}</span><small>Detik</small></div>`;
    }
    update();
...
function animateStats() {
    gsap.utils.toArray(".stat-num").forEach(stat => {
        ...
        let obj = { count: 0 };
        gsap.to(obj, { count: target, duration: 2, ease: "power2.out", onUpdate: () => { stat.textContent = Math.floor(obj.count).toLocaleString(); }, scrollTrigger: { trigger: ".hero-stats", start: "top 80%", toggleActions: "play none none reverse" } });
    });
}
...
function setupEventListeners() {
    ...
    function showWishlistModal() {
        ...
        if (state.wishlist.length === 0) wishlistBody.innerHTML = '<div style="text-align: center; padding: 40px;"><i class="far fa-heart" style="font-size: 3rem; color: #ccc;"></i><p style="margin-top: 16px;">Belum ada produk di wishlist</p></div>';
        else {
            const wishlistProducts = state.products.filter(p => state.wishlist.includes(p.id));
            wishlistBody.innerHTML = wishlistProducts.map(p => `<div style="display: flex; gap: 12px; margin-bottom: 15px; padding-bottom: 15px; border-bottom: 1px solid #eee;"><img src="${p.image}" style="width: 60px; height: 60px; object-fit: cover; border-radius: 8px;"><div style="flex: 1;"><div style="font-weight: 600;">${p.name}</div><div style="color: var(--warm-walnut);">${formatRupiah(p.price)}</div><button class="remove-wishlist" data-id="${p.id}" style="background: none; border: none; color: #999; cursor: pointer; font-size: 0.7rem;">Hapus</button></div></div>`).join('');
            document.querySelectorAll('.remove-wishlist').forEach(btn => btn.addEventListener('click', () => { const id = parseInt(btn.dataset.id); state.wishlist = state.wishlist.filter(wid => wid !== id); saveWishlist(); showWishlistModal(); }));
        }
        wishlistModal.classList.add('open');
        gsap.fromTo(".modal-content", { scale: 0.8, opacity: 0 }, { scale: 1, opacity: 1, duration: 0.3, ease: "back.out(0.5)" });
        const closeWishlist = document.getElementById('closeWishlistModal');
        if (closeWishlist) closeWishlist.onclick = () => wishlistModal.classList.remove('open');
    ...
    if (resetFilter) resetFilter.addEventListener('click', () => { state.currentBudget = 5000000; state.currentCategory = 'all'; state.sortBy = 'default'; state.searchQuery = ''; if (budgetSlider) budgetSlider.value = 5000000; if (budgetValue) budgetValue.textContent = formatRupiah(5000000); if (categorySelect) categorySelect.value = 'all'; if (sortSelect) sortSelect.value = 'default'; document.getElementById('searchInput') && (document.getElementById('searchInput').value = ''); document.getElementById('mobileSearchInput') && (document.getElementById('mobileSearchInput').value = ''); renderCatalogProducts(); if (filterOptions) filterOptions.classList.remove('show'); showToast('Filter telah direset', 'success'); });
